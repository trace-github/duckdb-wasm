// hash_ext — shared function implementations
// Used by both the WASM static extension and the native loadable extension.

#include "hash_ext_ffi.h"
...
static void HashRowScalarFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    if (const_field != DConstants::INVALID_INDEX) {
        ...
        const auto &col_name = col_data[col_fmt.sel->get_index(0)];
...
// ---------------------------------------------------------------------------
// Scalar: hash_json(json VARCHAR, key VARCHAR) -> UINTEGER
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Scalar: hash_json_keys(name_list VARCHAR[], value_json VARCHAR) -> UINTEGER
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Table function: hash_json_keys_table(TABLE, name_list VARCHAR[]) -> (hash UINTEGER)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Table function: metric_table(TABLE, series_id_key, parent_keys, target_key)
// ---------------------------------------------------------------------------
    ...
    vector<string> &names)
{
    ...
    return_types = {
        LogicalType::VARCHAR,  // vals
        LogicalType::VARCHAR,  // series_id
        LogicalType::UINTEGER, // parent_match_hash
        LogicalType::UINTEGER, // target_match_hash
        LogicalType::UINTEGER, // target_value_hash
        bd->value_type,        // value  (pass-through)
        bd->count_type,        // count  (pass-through)
    };
...
// ---------------------------------------------------------------------------
// Scalar: json_typed_extract(json, key, type) -> STRUCT(varchar, bigint, double, boolean)
// ---------------------------------------------------------------------------
